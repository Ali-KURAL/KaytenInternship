...
static __INLINE void debounce(void)
{
	...
	uint8_t current_state = HAL_GPIO_ReadPin(B1_GPIO_Port,B1_Pin) == GPIO_PIN_RESET;
...
void button_task_func(void *argument)
{
  ...
  for(;;)
  {
		debounce();
		if(pressed == 1){
			osEventFlagsSet(buttonPressedHandle,1U);
			pressed = 0;
		}
//		int16_t count = 0;
//		if((HAL_GPIO_ReadPin(B1_GPIO_Port,B1_Pin) == GPIO_PIN_RESET) && pressed != 1)
//		{
...
void led_task_func(void *argument)
{
  ...
  for(;;)
  {
		osEventFlagsWait(buttonPressedHandle,1U,osFlagsWaitAny,osWaitForever);
		osEventFlagsClear(buttonPressedHandle,1U);
